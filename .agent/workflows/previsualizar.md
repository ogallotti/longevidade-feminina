...
Identifique em qual pasta da página você está trabalhando. Use os arquivos da pasta da página.

**IMPORTANTE:** Pastas com prefixo `_backup_` são versões antigas - NÃO inclua no commit/PR.

### OBRIGATÓRIO: Volte para a Raiz (The Root Rule)
Antes de rodar qualquer comando Git ou Netlify CLI (`netlify status`, `netlify init`, etc), você **DEVE** garantir que está operando na RAIZ do projeto (onde fica o `.git` e o `netlify.toml`). Se você estiver dentro da pasta da página, os comandos vão falhar ou desconfigurar o repositório.
